...
#Change this in case of version error:

globalpy_compileVersion = str(39)
...
def createmovefile():
	...
	if PyBiteAlreadyExistsCheck == False:
		...
		time.sleep(3)
		shutil.move(dir + "/__pycache__/" + trimmedfile + ".cpython-" + globalpy_compileVersion+ ".pyc", dir + "/PyBite/")
		time.sleep(2)
		os.chdir(dir + "/PyBite/")
		time.sleep(1)
		pycbase = os.path.splitext(trimmedfile + ".cpython-" + globalpy_compileVersion+ ".pyc")[0]
		os.rename(trimmedfile + ".cpython-" + globalpy_compileVersion+ ".pyc", pycbase + '.py')
	else:
